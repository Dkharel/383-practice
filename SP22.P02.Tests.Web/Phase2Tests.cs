...
using SP22.P02.Tests.Web.Helpers;

namespace SP22.P02.Tests.Web;
...
public class Phase2Tests
{
    ...
    [TestMethod]
    public async Task CreateProduct_NegativeSalesPrice_ReturnsError()
    {
        //arrange
        var webClient = context.GetStandardWebClient();
        var request = new Dto
        ...
        //assert
        httpResponse.StatusCode.Should().Be(HttpStatusCode.BadRequest, "we expect an HTTP 400 when calling POST /api/products with negative sales price");
    }
    ...
    [TestMethod]
    public async Task CreateProduct_WithZeroSalesPrice_Returns201AndData()
    {
        //arrange
        var webClient = context.GetStandardWebClient();
        var request = new Dto
    ...
    [TestMethod]
    public async Task UpdateProduct_NegativeSalesPrice_ReturnsError()
    {
        //arrange
        var webClient = context.GetStandardWebClient();
        var target = await GetItem(webClient);
        ...
        //assert
        httpResponse.StatusCode.Should().Be(HttpStatusCode.BadRequest, "we expect an HTTP 400 when calling PUT /api/products/{id} with negative sales price");
    }
    ...
    [TestMethod]
    public async Task UpdateProduct_WithZeroSalesPrice_Returns200AndData()
    {
        //arrange
        var webClient = context.GetStandardWebClient();
        var target = await GetItem(webClient);
